...
The implemenation uses the colo of the originating worker as the name/id for the DO. Reload your browser to see non-cold-start latencies. To activate a new container, pass a 'DO' query param like `?DO` or `?DO=<xx>`. Passing a lowercase `do=<xx>` query param will activate the non-default DO without a container.
